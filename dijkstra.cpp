...
node* generate_graph(std::string location){
	...
	if(input.is_open()){
		while(getline(input, line)){
			int end_id = (find_right(0, line) + 1);
			int new_identity = stoi(line.substr(0, end_id));
...
int main(){
	...
	int target_id;
	std::cout << "Which node should be the target? ";
	std::cin >> target_id;
	std::cout << "\n";
	if(target_id < 0 or target_id > (num_nodes-1)){
		std::cout << "invalid node selected\n";
		return 0;
	}
